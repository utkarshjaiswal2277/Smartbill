...
function BusinessAssistant({ profile, invoices, products }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get(STORAGE_KEYS.chat, false);
  ...
  const persist = async (msgs) => {
    try {
      await window.storage.set(STORAGE_KEYS.chat, JSON.stringify(msgs), false);
    } catch (e) {
      /* best-effort */
    }
  };
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [pr, inv, prof] = await Promise.all([
          window.storage.get(STORAGE_KEYS.products, false).catch(() => null),
          window.storage.get(STORAGE_KEYS.invoices, false).catch(() => null),
          window.storage.get(STORAGE_KEYS.profile, false).catch(() => null),
        ]);
        if (pr?.value) setProducts(JSON.parse(pr.value));
        if (inv?.value) setInvoices(JSON.parse(inv.value));
        if (prof?.value) setProfile(JSON.parse(prof.value));
      } catch (e) {
  ...
  useEffect(() => {
    if (!loaded) return;
    window.storage.set(STORAGE_KEYS.products, JSON.stringify(products), false).catch(() => {});
  }, [products, loaded]);
  useEffect(() => {
    if (!loaded) return;
    window.storage.set(STORAGE_KEYS.invoices, JSON.stringify(invoices), false).catch(() => {});
  }, [invoices, loaded]);
  useEffect(() => {
    if (!loaded) return;
    window.storage.set(STORAGE_KEYS.profile, JSON.stringify(profile), false).catch(() => {});
  }, [profile, loaded]);
